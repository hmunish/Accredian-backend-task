...
exports.signup = async (req, res) => {
  try {
    ...
    const errorMessage = [];
    if (!validator.matches(username, '^[a-zA-Z0-9_.-]*$')) { errorMessage.push('Invalid Username'); }
    ...
    if (isUserEmail) { return res.status(409).send({ message: 'EmailId already exist' }); }

    // Validating if username already exists in the database
    const isUserName = await User.findOne({ where: { username } });

    // If user exist return with 409 conflict response
    if (isUserName) { return res.status(409).send({ message: 'Username already exist' }); }
